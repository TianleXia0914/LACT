...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, is_training=True):
    ...
    # if not use_flash_rotary or not is_training:
    #     q_embed = (q * cos) + (rotate_half(q) * sin)
    #     k_embed = (k * cos) + (rotate_half(k) * sin)
    # else:
    #     cos = cos[0].squeeze(0)
    ...
    #     q_embed = apply_rotary_emb_func(q.transpose(1, 2), cos, sin)
    #     q_embed = q_embed.transpose(1, 2)
    #     k_embed = apply_rotary_emb_func(k.transpose(1, 2), cos, sin)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids, self.training)
        # [bsz, nh, t, hd]
