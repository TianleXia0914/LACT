...
import jsonlines
train_type=['1p','2i','2in','2p','3i','3in','3p','inp','pin','pni']
complete_type=['1p']
...
def process_logical_queries(logical_query, 
    ...
    if(qtype in complete_type):
        entity_set = list(filter(lambda x: x!=None, [e1, e2, e3]))
        relation_set = list(filter(lambda x: x!=None, [r1, r2, r3]))
        #hard_question_file_path = os.path.join(f"{output_path}",f"{qtype}_hard_question.txt")
        answer = temp_answers[logical_query]
        #all_answer=answers[logical_query]
        ...
        #all_answer_text=", ".join(map(str,all_answer))
        ...
        hard_file_path=os.path.join(f"{output_path}",f"{qtype}.jsonl")
        temp={
            'question':premise_question,
            'answer':answer_text,
            #'all_answer':all_answer_text,
            'id':idx
        }
        with lock:
            with jsonlines.open(hard_file_path,'a')as nf:
                nf.write(temp)
...
def main(data_path, output_path):
    ...
    queries = mergeDictionary(test_queries)
    #train_queries = mergeDictionary(train_queries,valid_queries)

    global answers,hard_answers,train_a,temp_answers
    ...
    hard_answers=mergeDictionary(test_hard_a,valid_hard_a)
    ...
    triplet_files = [os.path.join(f"{data_path}","train.txt"),os.path.join(f"{data_path}","valid.txt")]
